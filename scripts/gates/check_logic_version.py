...
# 而被无视的闸门等于没有闸门（docs/agents/ci-gates.md §3）
ENGINE_FILES = (
...
def check_engine_change_bumps_version() -> list[Violation]:
    ...
    touched = set(changed_files()) & set(ENGINE_FILES)
    before = file_at(base, VERSION_FILE)
    if not touched or before is None:
        # 基线里还没有这个引擎：这次是把它整个加进来，没有可比的旧口径
        return []
    after = file_at(diff_head(), VERSION_FILE)
    if after is None or _version(before) != _version(after):
        return []
    return [
        Violation(
            "改了抽取引擎却没改 LOGIC_VERSION",
            f"{diff_range()}：{'、'.join(sorted(touched))}",
            _BUMP,
        )
    ]
