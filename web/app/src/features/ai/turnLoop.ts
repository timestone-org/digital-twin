/**
 * @fileoverview 浏览器这一侧回合循环的**内核**：收流 → 派发客户端工具 → 把结果送回去。
 *
 * 与产品无关的那一半在这里；「每一轮信封里带什么」「认不出的帧归谁」「模型收嘴之后
 * 要不要代用户催一句」由门面决定（助手：`turnRunner.ts`；知识库对话：
 * `features/knowledgeChat/turnRunner.ts`）。两个门面共用这一份——各抄一份的话，
 * 「工具失败也要把结果送回去」这种规矩一定会有一边漏掉。
 *
 * 服务端跑到客户端工具那一步就把流结束了，待办交给我们；我们执行它们，再带着
 * 结果发下一次推进。一个回合因此可能是好几次 HTTP 往返（ADR-0023）。
 *
 * ⚠ 往返有上限。模型绕进「调一个工具 → 看结果 → 再调同一个」的循环时，
 * 每一步看起来都合理，只有总轮数拦得住它。
 *
 * ⚠ 工具失败**照样要把结果送回去**，而且要说清失败了。不送的话服务端那次
 * 调用永远没有答复，模型下一轮会被端点判成请求不合法。
 *
...
/** 模型收嘴之后门面要代用户催的那一句：时间线上注一句、再发一句。 */
export interface Nudge {
  note: string
  text: string
}

export interface LoopInput<Body extends LoopBody> {
  advance: (
    sessionId: string,
    body: Body,
    signal?: AbortSignal,
   ...
   * ⚠ 每一轮都重新问一次：攒一份在第一轮用的话，助手自己动过两下之后，
   * 它读到的还是动手之前那一屏。
   */
  envelope: () => Body
  userText: string
  ...
  /** 跑一个客户端工具。抛出即失败，失败照样回填。 */
  dispatch: (call: AssistantToolCall) => Promise<unknown>
  /** 内核认不出的帧（如助手的 `plan`）交给门面。 */
  onFrame?: (name: string, data: Record<string, unknown>) => void
  /** 模型收了嘴：门面觉得活没干完就给一句要代发的话，觉得完了给 null。 */
  nudge?: () => Nudge | null
  /** 往返上限，到顶就停下并如实告诉用户。 */
  maxRounds: number
}
 ...
 * 一路写进已经销毁的状态。
 * @param input 从哪推进、信封里带什么、怎么派发
 * @param sink 事件交给谁
 */
export async function runLoop<Body extends LoopBody>(
...
async function pump<Body extends LoopBody>(
  input: LoopInput<Body>,
  body: Body,
  sink: LoopSink,
): Promise<PumpOutcome> {
...
function handle<Body extends LoopBody>(
  ...
  if (name === 'error') {
    sink.onError(readText(data.message))
    return null
  }
  input.onFrame?.(name, data)
  return null
}

/**
 * 把一批待办跑完，成败都收成结果。
 * ⚠ 每一个都顺手记成一步交给界面：这几步是助手唯一真正改动东西的地方，
...
/** 把一个不确定的东西读成对象；不是对象给空表。 */
export function readObject(given: unknown): Record<string, unknown> {
...
/** 把一个不确定的东西读成字符串；不是字符串给空串。 */
export function readText(given: unknown): string {
