 ...
 */
import { onBeforeUnmount, ref, watch, type Ref, type ComputedRef } from 'vue'
...
export interface LiveValues {
  ...
  staleKeys: ComputedRef<Set<string>>
}
...
export function useLiveValues(sourceId: Ref<string>): LiveValues {
  ...
  const freshness = useSampleFreshness(channel.isConnected)
  ...
  function start(id: string): void {
    ...
    unsubscribe = channel.subscribe(collectTopic(id), (payload) => {
      ...
      for (const { nodeKey, sample } of decodePointItems(payload)) {
        if (!nodeKey.startsWith(`${id}:`)) continue
