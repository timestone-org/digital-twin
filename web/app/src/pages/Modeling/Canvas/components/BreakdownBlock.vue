 ...
 * ⚠ 分位点那一档（正态 QQ）走 qq 态散点：这张图要读的是「点偏离对角线多远」，
 * 而横条上一根都没有对角线可偏，51 个分位点会摊成 51 根谁也不看的横条。
...
import { buildQq, isQq } from '../scripts/qqPoints'
...
/** 分位点那一档（正态 QQ）；不是这一档就给 null，交回原来的横条画法。 */
const quantiles = computed(() =>
  isQq(props.block.payload) ? buildQq(props.block.payload) : null,
)
...
const isPlain = computed(
  () => kind.value === null && drift.value === null && quantiles.value === null,
)
</script>
...
<template>
  <section class="dt-ml-breakdown">
    ...
    />
    <ScatterPlot
      v-else-if="quantiles !== null"
      mode="qq"
      :series="quantiles.series"
      :x-label="quantiles.xLabel"
      :y-label="quantiles.yLabel"
      :note="quantiles.note"
    />
    <ScatterPlot
