 ...
 * ⚠ 对比图那一区再分主体位与辅图格（规格 §3.2）：两位共用 44rem 那一档宽度，
 * 辅图格摆得下两张整图才排两列。图上的刻度字随 `viewBox` 等比缩，格子一窄它就
 * 跟着小一档——故列宽下限不是版式偏好，是这张图读不读得出数的分界线。
...
.dt-ml-blocks {
  ...
  // 辅图格的列宽下限与主体图同档：`svg` 的 viewBox 把 7px 的刻度字按「渲染宽
  // ÷ 360」等比缩，列一窄字就跟着小一档。⚠ 列宽下限写小了，弹窗越宽反而越糟——
  // auto-fit 会多排一列，实测 1084px 的辅图格里排三列、每张图只剩 353px，刻度
  // 字 6.87px，比图注那行 10px 的字还小。摆得下两张 44rem 的图才排两列
