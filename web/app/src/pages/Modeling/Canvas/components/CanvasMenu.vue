 ...
 * Esc / 点菜单外 / 画布滚动都收起。条目与置灰由 `../scripts/menuItems` 算好。
 *
 * ⚠ window 监听用 AbortController 持有，收起与卸载都 abort：画布一开就是几小时，
 * 漏一次就留下一副永远跟着鼠标走的监听。
 * ⚠ 键盘事件在捕获阶段拦下并停传：画布的全局快捷键也挂在 window 上，不停传的话
 * Esc 会在关菜单的同时把选中一并清掉。
 */
import { computed, nextTick, onUnmounted, ref, watch } from 'vue'

import type { MenuAction, MenuGroup } from '../scripts/menuItems'
import { clampMenu } from '../scripts/menuPosition'

const props = defineProps<{
  menu: { at: { x: number; y: number }; groups: readonly MenuGroup[] } | null
}>()

const emit = defineEmits<{ pick: [action: MenuAction]; close: [] }>()
...
const style = computed<Record<string, string>>(() => {
  ...
  if (menu === null) return {}
  const items = menu.groups.reduce((sum, one) => sum + one.items.length, 0)
  const at = clampMenu(
    menu.at,
    { width: window.innerWidth, height: window.innerHeight },
    { items, groups: menu.groups.length },
  )
...
function focusBy(step: 1 | -1): void {
  ...
  const first = step === 1 ? 0 : items.length - 1
  const next =
    current < 0 ? first : (current + step + items.length) % items.length
  items[next]?.focus()
}
...
<template>
  <Teleport v-if="props.menu !== null" to="body">
    <div
      ref="rootEl"
      class="dt-ml-menu"
      role="menu"
      ...
      @contextmenu.prevent
    >
      <template v-for="(group, index) in props.menu.groups" :key="group.key">
        <hr v-if="index > 0" class="dt-ml-menu__sep" role="separator" />
        <button
          v-for="item in group.items"
          :key="item.action"
          type="button"
          class="dt-ml-menu__item"
          :class="{ 'dt-ml-menu__item--danger': item.danger }"
          role="menuitem"
          :disabled="item.disabled"
          @click="emit('pick', item.action)"
        >
          <span class="dt-ml-menu__label">{{ item.label }}</span>
          <span v-if="item.keys !== ''" class="dt-ml-menu__keys">
            {{ item.keys }}
          </span>
        </button>
      </template>
