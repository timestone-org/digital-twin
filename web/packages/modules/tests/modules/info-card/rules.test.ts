/**
 * @fileoverview 守 info-card 自带的值规则表：规整这道门、声明序取首个命中、规则自己的
 * 颜色盖掉语义色，以及配置字段的形状。
...
import {
  ...
  normalizeValueRules,
  valueRulesField,
} from '../../../src/modules/info-card/rules'
...
describe('声明序取首个命中', () => {
  const water = [
    { op: 'gt', value: 55, level: 'danger', color: 'var(--state-danger)' },
    { op: 'gt', value: 45, level: 'warning', color: 'var(--state-warning)' },
    { op: 'lt', value: 30, level: 'info', color: 'var(--accent-primary)' },
    { op: 'between', value: 30, value2: 45, level: 'normal', color: '' },
  ]

  it('两档都超时命中最严重的那一条——高危必须排在预警之前', () => {
    expect(hitOf(60, water)?.level).toBe('danger')
  })

  it('只超一档时命中那一档', () => {
    expect(hitOf(50, water)?.color).toBe('var(--state-warning)')
  })

  it('区间档也算命中，正常也是一档结论', () => {
    const hit = hitOf(35, water)
  ...
  it('颜色留空时回落该严重度的语义色', () => {
    expect(hitOf(20, [{ op: 'lt', value: 30, level: 'info' }])?.color).toBe(
      'var(--state-info)',
    )
  })
  ...
  it('等于那一档给工单状态这类离散值用', () => {
    ...
    expect(hitOf(1, states)?.label).toBe('执行中')
    expect(hitOf(2, states)).toBeNull()
  })
...
describe('配置字段', () => {
  ...
  it('键与标签可以改，同一块上摆两张表时用得着', () => {
    const other = valueRulesField('subRules', '副读数规则', '判副读数')

    expect(other.key).toBe('subRules')
    expect(other.label).toBe('副读数规则')
    expect(other.help).toBe('判副读数')
  })
