/**
 * @fileoverview info-card 自带的值规则表：行比 `shared/thresholds.ts` 的多一个 `color`，
 * 求值口径仍是那一份——逐条调 `evaluateThresholds` 取声明序里的首个命中，再用规则自己的
 * 颜色盖掉语义色（MODULE_INFO_CARD_DESIGN §3）。
 * ⚠ 与 info-list 的同名文件各带一份、**不许互相 import**：一个模块一个目录是可达集扫描
 * 的前提，跨目录取件会让「这个模块用到了哪些文件」再也数不清。
  ...
  return {
    ...
    itemSchema: [
      ...
      },
      {
        ...
        type: 'color',
        default: '',
        help: '留空跟随严重度语义色；水温梯度、能源类型这类非严重度的配色填这里。',
      },
