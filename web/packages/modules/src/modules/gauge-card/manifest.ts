...
export default defineModule({
  type: 'gauge-card',
  description:
    '仪表卡片：一块摆 1..N 个带量程的读数，五档几何（弧度盘 / 横向条 / 目标轨道 / 储罐 / 温度计）共用「量程 → 百分比 → 填充」同一条链，可画刻度、量程端点与目标标记。读数要表达「离满还有多远」「完成了多少」「液位到哪儿」时用它；只要裸数字加单位用 metric-card，要带图标与涨跌对比的卡片网格用 info-card，要一行里带副读数与徽章的清单用 info-list。一个数组绑定槽 `gaugeValues`，行钉在 `items` 配置项上：第 i 行喂第 i 个仪表，子槽 `value` 是主读数、`aux` 是目标值的实时来源（绑了就顶掉行内那个静态「目标值」），都收数值；量程上下限、单位、小数位是逐项的配置，不从点位来。⚠ 每一项的「量程上限」必须大于「量程下限」，否则这一个仪表既不画填充也不伪造 0%；「目标值」留空 = 不画目标标记且完成率退回按量程算。⚠ 删掉 `items` 中间一项，它之后每一个仪表的绑定都会改喂前一个。',
  displayName: '仪表卡片',
  category: '数据',
  icon: 'gauge',
  keywords: [
  ...
  configSchema: [
    ...
    },
    {
      ...
      default: 'solid',
      span: 'half',
      help: '渐变档由填充色自己调深浅，不另开色标：弧与条向前加深，储罐自下而上淡出。',
