...
export default defineModule({
  type: 'info-list',
  description:
    '信息列表：一块摆 N 行，每行由「前导列 ｜ 最多三段左右分列的行段 ｜ 两个尾列」声明式编排，行内可挂徽章、分类标签、两条进度件、命中文案、描述与时刻，另有一条有值才出的扩展指标行；整块还能按行内「分组」分段或分页签，按严重度排序，只看命中规则或只看告警的行。一屏里要竖着列十几个点位、每行还带副读数 / 状态 / 进度条时用它；一格一个大数字的网格用 metric-card 或 info-card，带量程图形用 gauge-card，条目由后端推送、条数不固定的用 info-feed。一个数组绑定槽 `listValues`，行钉在 `items` 配置项上：第 i 行喂第 i 项，子槽十一个——`value` 主读数、`aux` / `aux2` / `aux3` 副读数、`extra1` / `extra2` / `extra3` 扩展指标、`status` 设备状态（收数值，不要配 enum 映射）这八个收数值，`name` 行名、`text` 描述、`time` 时刻文本这三个收字符串；单位、小数位、量程与目标值是逐行的配置项。⚠ 删掉 `items` 中间一项，它之后每一行的绑定都会改喂前一行。',
  displayName: '信息列表',
  category: '数据',
  icon: 'table',
  keywords: [
