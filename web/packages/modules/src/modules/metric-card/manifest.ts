...
export default defineModule({
  type: 'metric-card',
  description:
    '实时数值：一块摆 1..N 个点位读数，一格是「名称 + 大字读数 + 单位 + 状态点」，带四段阈值（危险下限 / 预警下限 / 预警上限 / 危险上限）与逐格四档取数状态（未绑定 / 等待首帧 / 取不到 / 有读数）。墙上「看一眼现场此刻是多少」的首选，最朴素、字最大；要图标、渐变数字与涨跌对比用 info-card，要量程 / 进度 / 液位那种图形用 gauge-card，要一行里带副读数与徽章的清单用 info-list。一个数组绑定槽 `itemValues`，行钉在 `items` 配置项上：第 i 行喂第 i 个指标，唯一子槽 `value` 收数值；单位、小数位、真假文案与四条阈值边界都是逐项的配置，不从点位来，开关量档按「非 0 即真」判（工控点位的开关量多半是 0/1 数值而不是 JSON 布尔）。⚠ 四条阈值边界都是开区间：`warnAbove: 80` 的意思是超过 80 才预警，等于 80 不预警；四条全留空时连状态点都不画——没有判据就连「正常」都不该说。⚠ 删掉 `items` 中间一项，它之后每一行的绑定都会改喂前一个指标。⚠ 两个键名与兄弟模块不一致，照着它们写会静默不生效：千分位在这里叫 `grouping`（别的模块叫 `thousands`），逐项联动值在这里叫 `items[].key`（别的模块叫 `emitValue`）。',
  displayName: '实时数值',
  category: '数据',
  icon: 'gauge',
  ...
  configSchema: [
    ...
    },
    {
      ...
      itemSchema: [
        ...
        },
        {
          ...
          type: 'string',
          default: '',
          // ⚠ 同上：兄弟模块的同一项叫 emitValue，存量配置里存的是 key
          help: '点这一格时上抛的值，留空则这一格点了不上抛。与其他数据模块的「联动值」同义，但这个模块的键名是 `key` 而不是 `emitValue`。',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      default: false,
      span: 'half',
      // ⚠ 键名与三个兄弟模块不一样（那边叫 thousands）。存量大屏里存的就是
      //   `grouping`，改名会让配过它的节点静默失去千分位，故只在这里点明
      help: '与信息卡片 / 仪表卡片 / 信息列表的「千分位分隔」是同一件事，但这个模块的键名是 `grouping` 而不是 `thousands`。',
    },
    {
      key: 'emptyText',
      label: '无数据占位',
