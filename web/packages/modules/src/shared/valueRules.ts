/**
 * @fileoverview 值规则表：行比 `shared/thresholds.ts` 的多一个 `color`，求值口径仍是
 * 那一份——逐条调 `evaluateThresholds` 取声明序里的首个命中，再用规则自己的颜色盖掉
 * 语义色（MODULE_INFO_CARD_DESIGN §3）。列表族与卡片族共用这一份。
...
/**
 * 生成「值规则」配置字段，模块直接铺进自己的 `configSchema`。
 * ⚠ 颜色只填 `var(--…)` 引用：算出来的色值会原地钉死在一套配色上，换肤时不跟着走。
 * @param key 字段键
 * @param label 字段标签
 * @param help 标签旁的说明
 */
  ...
  return {
    ...
    itemSchema: [
      ...
      },
      {
        ...
        type: 'color',
        default: '',
        help: '留空跟随严重度语义色；能源类型、水温梯度这类非严重度的配色填这里。',
      },
