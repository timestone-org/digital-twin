...
export function useSceneTools(options: SceneToolsOptions): SceneTools {
  ...
  function boxOf(names: readonly string[]): THREE.Box3 | null {
    const index = options.nodeIndex()
  ...
  function locate(hit: TwinSearchHit): void {
    const core = options.core()
    const box = boxOf(hit.nodes)
    // 命中的实体在模型里找不到对应几何：不动镜头，也不报错——那是配置与模型
    // 对不上，由诊断面板去说，这里乱飞一下只会让人更糊涂
  ...
  function applyClipping(): void {
    ...
    const lo = box.isEmpty()
      ? Number.NaN
      : box.min[axis === 'none' ? 'x' : axis]
    const hi = box.isEmpty()
      ? Number.NaN
      : box.max[axis === 'none' ? 'x' : axis]
    const plane = clipPlaneFor(axis, clipRatio.value, lo, hi)
  ...
  function addPoint(point: THREE.Vector3): void {
    const next: Point = [point.x, point.y, point.z]
    // 已经有两点了就从头开始量，而不是往后堆
    points.value = points.value.length >= 2 ? [next] : [...points.value, next]
  }

  function interceptClick(event: PointerEvent): boolean {
    const core = options.core()
    ...
    if (hit !== null) addPoint(hit.point)
    return true
  }

  // ⚠ 轴或位置一改就重算：交给宿主去调的话，漏一处的表现是「拖了滑块没反应」
  watch([clipAxis, clipRatio], applyClipping)
