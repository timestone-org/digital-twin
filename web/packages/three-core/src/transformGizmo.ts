...
export class TransformGizmo {
  ...
  /**
   * 把手柄挂到一个实体上。
   * @param target 实体的位置与朝向；null = 收起手柄
   * @param mode 平移还是旋转；旋转只对箭头有意义
   */
  attach(target: GizmoTarget | null, mode: GizmoMode = 'translate'): void {
    ...
    this.proxy.quaternion.copy(orientationOf(target.direction))
...
/** 朝向 → 四元数；没有朝向的实体用单位四元数。 */
function orientationOf(direction: Vec3 | null): THREE.Quaternion {
  const quaternion = new THREE.Quaternion()
  if (direction === null) return quaternion
