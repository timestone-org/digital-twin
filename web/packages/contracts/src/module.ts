...
export interface ModuleMeta {
  ...
  interactive?: boolean
  /**
   * 由联动规则推出的「此刻该选中哪个值」：本节点的按值跳转规则里，目标正是
   * 当前这张屏的那一条路由的值。选择类控件据此高亮「现在在哪一格」。
   * ⚠ 它是唯一能让一条页签栏认出自己所在那张屏的东西——没有它，当前格只能
   * 每张屏各配一次静态下标，配漏一张就表现成「点了不换屏」（点的正是自己那格）。
   * 推不出（没有按值跳转规则 / 没有一条指向本屏）时缺席。
   */
  activeValue?: string
}
