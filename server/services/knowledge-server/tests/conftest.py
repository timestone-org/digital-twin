...
本服务的能力面不碰数据库，故这一层的用例不需要真库；碰库的用例自己挂
`requires_postgres`。
...
# 身份头的存活时长，取值本身不参与断言
HEADER_TTL_S = 900
FULL_CODES = (KNOWLEDGE_USE, KNOWLEDGE_WRITE, KNOWLEDGE_MANAGE)
# 占位取值：本层用例一个都不连
...
async def app_client(
    ...
    async with httpx.AsyncClient(
        transport=transport, base_url="http://knowledge-test", timeout=10
    ) as client:
