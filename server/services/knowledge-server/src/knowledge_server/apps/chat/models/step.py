...
⚠ 步骤单独一张表而不是塞进消息的 JSON 里：界面要逐条渲染并可折叠；续跑只需要
读最后一条待续的那一步；排障时「哪一步慢、哪一步失败」是最常问的问题。
"""
...
from knowledge_server.apps.chat.enums import STEP_KINDS, STEP_STATES, sql_values
...
class ChatStep(UuidPrimaryKeyMixin, TimestampMixin, Base):
    ...
    # 结束的时刻，而 `created_at` 量的是行什么时候插进来的
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["message_id"],
            [f"{DB_SCHEMA}.kb_chat_messages.id"],
        ...
        Index("ix_kb_chat_steps_message_id", "message_id"),
        CheckConstraint(
