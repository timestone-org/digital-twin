...
数据绑定的配置面）与**采集配置面**（数据源、点位、采集计划下发、点位历史读侧），以及
**分析建模**（用流水线把台账数据训成模型，再经公式库条目回到台账列）。

分析建模的完整规格见 [`docs/MODELING_DESIGN.md`](../../../docs/MODELING_DESIGN.md)。
...
| 词 | 指什么 | 不叫什么 |
|---|---|---|
| **流水线** `pipeline` | 一张有向无环图：节点是算子实例，边是数据流。它是**定义**，不含任何一次运行的结果 | 不叫工作流 |
| **算子** `operator` | 图上的一类计算单元，是**代码**不是数据——用户只能实例化已有算子并配参数 | 不叫模块（`module` 已被大屏组件系统占死） |
| **端口** `port` | 算子的具名输入 / 输出口。边**必须**同时指明两端的端口名 | —— |
| **契约** `contract` | 端口上负载的类型标识，如 `frame@v1`。两端不相等即不许连线，这是唯一的类型判据 | 不叫数据类型 |
| **特征帧** `frame` | 流水线里流动的唯一表格载体：列定义（含角色）+ 行矩阵 + 时间索引 | 不叫数据集（那已经指台账那一层） |
| **运行** `run` | 流水线的一次执行。有自己的图快照、状态机与节点级记录，是**过程记录**，有保留期 | 不叫任务 |
| **结果摘要** `preview` | 一个输出端口的**有硬上限**的可视表示，进数据库 | —— |
| **模型版本** `model version` | 一次成功运行产出的、**不可变**的可服务表示。发布后只读，改要发新版本 | —— |
| **绑定** `binding` | 一条公式库条目与一个模型版本的对应关系 + 形参到特征列的有序映射 | 不是部署：绑定供台账内部求值 |
| **部署** `deployment` | 一个模型版本面向第三方的稳定 URL、部署密钥与配额 | 不是绑定；两者可独立存在 |

⚠ **别名只做展示**：上下文寻址、边的两端、节点级记录一律用节点 `id`。
⚠ **一条流水线同时只能有一次在途运行**，这是数据库上的部分唯一索引，不是应用层的锁。
...
### 2.2 采集配置面的不变量（[ADR-0001](../../../docs/adr/0001-采集运行时独立成服务而配置面留在平台.md)）
...
26. **采集计划的版本号是内容摘要**，不是 `max(updated_at)`。删掉一个点位不会让任何
    ...
    collector 就永远不会拿新凭据重连（ADR-0001）。
...
### 2.3 大屏实时发布的不变量（[ADR-0005](../../../docs/adr/0005-实时通道与边缘网关的职责分界.md)、[ADR-0049](../../../docs/adr/0049-大屏运行态取数统一走WS.md)）

29. **只推有人在看的大屏**，活跃集合由 hub 的订阅关系推导，见下面第 4.2 节的
    「活跃大屏怎么来」。
30. **新观看者出现推一次实时点位全量**，这一路零 HTTP——首帧初值也走 WS。
    归档与台账等按需数据仍走各自 HTTP provider。新观看者判据是**连接集合变大**
    而不是人数变化：人数不变的换人同样是一位新观看者。
...
保管业务状态**，[ADR-0005](../../../docs/adr/0005-实时通道与边缘网关的职责分界.md)
§理由一点名反对。
...
## 模型供应商目录（`apps/llm_providers`，ADR-0041）
...
| **模型** `llm_model` | 供应商上登记的一个代号：种类 `chat` / `embedding` / `rerank`、接不接图、嵌入维数 | 不是画布的「模块」 |
| **重排线形** `rerank_dialect` | 重排端点说哪一套线形（`jina` / `dashscope`），配在**供应商**上 | 不叫厂商——同一套线形有一大把端点在说 |
| **用途** `purpose` | 消费方的一个取模型的口子（`assistant.chat`、`knowledge.embedding` …），闭合集合 | 不叫场景 / 通道 |
| **分配** `llm_assignment` | 一个用途指到一路供应商上的一个模型 | 不叫绑定（那是大屏的词） |

三条不变式：

1. **密钥只以密文入库、只以尾巴出门。** 明文只在内部目录（`/internal/v1/platform/llm-catalog`，
   服务级密钥）上走一跳给两个消费方。
2. **分配时用途、模型与种类要对齐**：嵌入用途不许指对话模型，看图用途不许指不接图的。
...
6. **重排线形跟着端点走，不跟着模型走**（ADR-0041）：它说的是「打哪个路径、
   什么请求体」。挂在模型上的话，同一路上配两个重排模型就能配出两套互相矛盾的
   线形，而只有一套打得通。没配这一格时按默认那一路解。
7. **用途面下发 `has_env_default`**：没分配时那一侧还有没有环境变量兜底。
   为假的那一路（`knowledge.rerank`）界面上说的是「这套部署不启用这一路」——
   说成「沿用环境变量」的话，人会去翻一个根本不存在的配置项。

⚠ 用途码在助手（`llm/ports.py`）与知识库（`llm_purposes.py`）各复述一份，形态码在
助手与 `llmcore` 各一半，重排线形码在 `llmcore/rerank/` 与前端各一份；
逐字一致由前端 `llm-shapes.contract.spec.ts` 对着源码比。
加一档用途或一种形态 = 几处字面量 + 一次放宽 CHECK 的迁移，且**一定要有一个消费方
真接得了它**。

## 报告模板与自动生成

`apps/report` 是台账的表达层：**模板**保存正文、指标和页面设置；**报告期**按业务
时区限定数据窗口；**生成记录**保存不可变模板快照、执行状态与产物；**定时规则**
驱动周期出稿。它只通过 dataset 的公开服务面读取生效值，不接设备和点位历史。

模板采用乐观版本保护。手动与定时生成共用 worker 管线，定时按规则与报告期唯一，
提交在事务成功后入队。报告数字对外为字符串，求值同时保留数值类别；截断及陈旧
数据必须标明。权限分查看、管理模板、生成下载、管理规则四档。
设计及启用步骤见 `docs/REPORT_TEMPLATE_DESIGN.md`。
