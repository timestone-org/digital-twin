...
| **绑定** `binding` | 一条公式库条目与一个模型版本的对应关系 + 形参到特征列的有序映射 | 不叫部署（这里没有独立进程，推理就发生在台账重算的那个协程里） |
...
### 2.3 大屏实时发布的不变量（[ADR-0005](../../../docs/adr/0005-实时通道与边缘网关的职责分界.md)、[ADR-0007](../../../docs/adr/0007-实时通道薄化与开放主题命名空间.md)、[ADR-0049](../../../docs/adr/0049-大屏运行态取数统一走WS.md)）
...
6. **重排线形跟着端点走，不跟着模型走**（ADR-0042）：它说的是「打哪个路径、
