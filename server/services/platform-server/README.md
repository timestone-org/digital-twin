...
业务平台。当前只有一个功能模块 `apps/hvac`：**空调台账**与**车间 / 房间空间配置**。
...
```bash
cp .env.example .env      # 填 Postgres 与 PLATFORM_EDGE_SIGNING_SECRET
...
```bash
uv run pytest                       # 四层测试
uv run platform-openapi             # 重新导出 openapi.json
uv run platform-openapi --check     # 与代码比对（CI 用）
uv run alembic upgrade head
