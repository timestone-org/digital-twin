"""训练管线与工件护栏的用例 —— 用可学的合成数据把整条链走通。

合成口径：达标时长 ≈ 8 × 温度超限量 + 台数补偿。模型只要能把「超得多 →
用时长」学出来，折外指标就该明显好于瞎猜。
"""
...
def sample(index: int) -> EpisodeSample:
    """第 index 条合成事件：温度超限量循环 1–6 度，时长由它线性决定。

    Args: index。
    """
    over = 1.0 + (index % 6)
    running = ("K11",) if index % 3 else ("K11", "K12")
    duration = int(8 * over - (4 if len(running) == 2 else 0))
    return EpisodeSample(
        conditions=StartConditions(
            ...
            idle_minutes=390,
            readings={
                "K11": {"workshop_temp_avg": 26.0 + over},
