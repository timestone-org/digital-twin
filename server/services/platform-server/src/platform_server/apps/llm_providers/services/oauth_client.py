...
from platform_server.apps.llm_providers.errors import (
    LlmLoginRejected,
    LlmLoginUpstreamUnavailable,
)
from platform_server.apps.llm_providers.services.tokens import (
    CLAIMS_NAMESPACE,
    TokenBundle,
)
...
class OAuthClient:
    ...
    async def start_device_code(self) -> DeviceCodeStart:
        ...
        if start is None:
            raise LlmLoginRejected(_why(body, "上游没给出可用的用户码"))
        return start
    ...
    async def poll_device_code(
        ...
        if response.is_success:
            ...
            if grant is None:
                raise LlmLoginRejected(_why(body, "上游没给出可用的授权"))
        ...
        raise LlmLoginRejected(_why(body, "登录没有完成，请重新开始"))
    ...
    async def _send(
        ...
        try:
            ...
            raise LlmLoginUpstreamUnavailable("登录服务此刻连不上") from error
    ...
    async def _posted(
        ...
        if response.is_success:
            return read
        raise LlmLoginRejected(_why(read, "登录没有完成，请重新开始"))
...
def _bundle_of(
    ...
    if not (isinstance(access, str) and isinstance(refresh, str)):
        raise LlmLoginRejected(_why(body, "上游没给出可用的令牌，请重新登录"))
    expires_in = _int_or(body.get("expires_in"), 0)
    if expires_in <= 0:
        # 存一份立刻就过期的令牌，等于每次对话都先失败一次再去刷新
        raise LlmLoginRejected("上游给的令牌没有有效期，请重新登录")
