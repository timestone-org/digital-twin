...
async def converge_pipeline(
    sessions: Sessions, *, pipeline_id: uuid.UUID, keep: int
) -> int:
    """把一条流水线的节点级明细收敛到最近 `keep` 次运行，回删了几条。

    ⚠ 每次运行结束时调一次就够，不必另起定时任务：删的永远是「这条流水线的
    老运行」，范围有界。
    Args: sessions, pipeline_id, keep。
    """
...
async def sweep_expired(
    sessions: Sessions, *, options: RetentionOptions
) -> int:
    """删掉过期的运行行，回删了几条。
    ...
    async with sessions.session() as session:
        ...
        return await run_crud.delete_by_ids(session, expired)
...
class ModelingRetention:
    ...
    def __init__(
        self, *, sessions: Sessions, options: RetentionOptions
    ) -> None:
    ...
    async def _sweep(self) -> None:
        ...
        try:
            ...
            expired = await sweep_expired(self._database, options=self._options)
            _logger.info(
