...
⚠ 「配了」不等于「能用」：要登录的那几路还得先登录过，而登录态在 platform
那一侧（ADR-0041），只按配置答不出来——所以这两问都要问一次平台。
...
class LoginProbe(Protocol):
    ...
    ⚠ 登录态在 **platform** 那一侧（ADR-0041），所以这一问是一次跨进程调用；
    每一路各问一次，而要登录的路一般只有一两条。
    """
...
async def profiles_of(
    ...
    ⚠ 要登录的那几路的「能不能用」要**去问一次登录态**：只按配置回答的话，
    界面上会摆出一个点了就报错的选项，而报出来的错是「模型暂时不可用」。

    Args: models, credentials（登录态探针；用例可不接）。
    """
