...
⚠ 「配了」不等于「能用」：订阅账号那一路还得先登录过，而登录态在库里，
...
from ai_assistant.llm import CODEX_PROFILE, ModelRegistry
...
async def _codex_connected(credentials: LoginProbe | None) -> bool:
    if credentials is None:
        return False
    return (await credentials.status(CODEX_PROFILE)).is_connected
