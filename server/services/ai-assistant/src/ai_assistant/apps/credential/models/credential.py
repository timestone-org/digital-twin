"""模型凭据表。**这一期只留声明，没有任何读写路径**（ADR-0041）。

登录态已经归 platform 的 `llm_provider_credentials` 持有。这张表按扩展—收缩两次
发布留着——代码可回滚、数据库不回滚，故「新结构 + 旧代码」必须可用；删表在下一次
发布。留着的这几行密文没有人再解，存量令牌也不搬。
"""
