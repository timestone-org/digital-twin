...
⚠ 那个端点只认 `^[a-zA-Z0-9_-]+$` 的工具名，而两个消费方的工具名都用点分命名
空间（`points.search` / `kb.search`）。原样发过去是一条 400——`Invalid
'tools[0].name': string does not match pattern`——它既不说是哪个工具，也不说
问题出在点号上（实测）。按量计费那一路收这样的名字，所以这件事只在这一路上发生。

⚠ **历史里的工具调用也要换。** 上一轮的 `tool_calls` 原样重放同样是 400，
那时的现象是「这个会话从某一轮起再也说不了话」，而新开的会话好好的。

⚠ 换回来是按 `__` 反着切，故规范名里不许出现 `__`——消费方各有一条用例钉着
自己那整册工具名。
"""
