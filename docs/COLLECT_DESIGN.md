...
ValueSink(point_code, value, ts_ms, quality)      ← 缝在这里，以下全部协议无关
  ...
  └─→ ③ 归档准入：archive_enabled ∧ (首值 ∨ 心跳到期 ∨ 超死区 ∨ 值变了)
          ...
          ③' 心跳补发：每秒扫一遍**在线且订阅着**的数据源，上一条归档已满一个
             archive_max_interval_ms 的点位，主动补一行「值还是这个」
  │
...
**③' 心跳是主动补的，不是等出来的。** ③ 里「心跳到期」那条准入只在驱动送来一条
读数时才判，而订阅**只在值变了才回调**：一个稳定的点位在 ③ 里永远等不到心跳，
库里就是几个月一行，台账按桶折算的每一档（`last` 也不例外）都取不到它。所以归档
缓冲每秒扫一遍 supervisor 报「在线且订阅着」的数据源，上一条归档已满一个
`archive_max_interval_ms` 的点位就主动补一行，值取该点位最近**见过**的读数
（死区吞掉的那点变化也算）。四条口径：

- **时间戳落在「锚 + 整数个心跳」的网格上，不抄本地时钟。** 锚是最后一条实测归档
  读数的时刻与收到它的本地时刻。抄本地时钟有两处坑：设备时钟与本地有偏差时，心跳
  行会排到下一条实测读数之后，台账取末值取到的是陈值；「到期才发、发时取当下」
  会因扫描抖动一格比一格晚，隔一阵就整个跳过一个台账桶。网格保证**相邻两行间隔
  恰好一个心跳**，任何不窄于心跳的桶里都至少一行——台账周期必须 ≥ 心跳。
- **读数的时刻离本地此刻超过一个心跳，锚改用本地时钟。** 订阅的初值带的是上一次
  变化的时刻（可能是几天前），从它外推的心跳会整批落进过去、撞主键而静默丢掉。
- **掉线的数据源不补，且忘掉它「见过」的读数。** 重连之前谁也不知道现场的值还是
  不是这个；重连后订阅把初值重新推一遍，从那一条起再补。丢主拆掉会话之后同理——
  否则是一个非 leader 在造行。
- **轮询不补。** 每一轮都把读数送进 ③，心跳到期那条准入自己会收。
- 停摆超过两个心跳只补最近一格，不倒着补：那段时间没人在看现场。
