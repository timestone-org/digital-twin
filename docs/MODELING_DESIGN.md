...
| 路由 | 名称 | 版式 | 权限 |
| --- | --- | --- | --- |
| `/modeling/pipelines` | 分析建模（流水线列表） | 常规宽度，表格 / 卡片双档 | `modeling:view` |
| `/modeling/pipelines/:pipelineId` | 建模画布 | 满屏 | `modeling:view` |
| `/modeling/pipelines/:pipelineId?run_id=…` | 运行回看（同一组件，只读态） | 同上 | `modeling:view` |
| `/modeling/models` | 模型版本与绑定 | 满屏表格 | `modeling:view` |

⚠ 落地时列表路径带上了资源段（`/modeling/pipelines` 而不是 `/modeling`）：模型库
与流水线是**平级**的两种资源，让前者去当后者的子路径会读成「模型属于某条流水线」，
而一个版本一旦发布就脱离了产它的那条流水线。
...
⚠ 二期这条**写了但全仓零调用**（端点在、`issues` 状态在、提示条也在，就是没人调），
于是列引用、空台账这类问题只有按下「运行」才由后端拦下，而那条 400 只带一句
「流水线还有问题，先改好再运行」——逐条定位信息在信封的 `details` 里，
而前端的 `describeError` 只取 `message`，用户看到的就是四个字。三条一起才算接上：
**图一改就防抖校验**（400ms，卸载时清定时器 + 作废在飞的那次）、
**问题逐条列出且点得进对应那张卡片**、**运行前再校一次，不通过就不发起**。
⚠ 校验的必须是**请求体里那张图**：端点原本不收请求体、只校验库里那份，而前端一直在
传 `{graph}`——线形对不上，但因为没人调用，两边都没红过。
...
- `x-dt-widget:"table"` 的候选是 `GET /dataset-tables` 的真实清单，按页翻全。下拉**总带
  搜索框**（名称与编码都能搜，不看选项多少）；图里存着却不在清单里的编码仍列为一项并标注，
  免得空着显示「请选择」。
  ⚠ 那条接口归 `dataset:view` 管，而建模是另一套权限码：只有 `modeling:*` 的人会吃 403。
  那时**不能只显示一个空下拉**——空下拉读起来是「一张台账都没建」，而真相是「你看不到」，
  两者的处置完全不同。没有那个码就不发请求、退回手填编码并把原因原话说出来。
  **拉取失败是另一种状态**：给「重试」而不是手填，也不把它说成权限问题。
...
- `x-dt-widget:"column"` 的候选是**上游那条支路**上的取数节点选的台账的列，
  **再按那些取数节点自己挑的列收窄一次**。
  ...
  ⚠ 收窄口径与后端 `graph_walk.known_keys_by_node` **一一对应**：取数挑过列时只剩那几列，
  留空（= 取全部列）时不收窄，两支里有一支留空则整体不收窄。二期漏了这一步，
  现场的表现是「取数必须把列选全，否则显示配置错误」——真相是取数一改窄，下游那个节点
  还留着窄之前勾的列，而报错落在下游、只说「上游没有列 F1」。因此三条一起才算修好：
  候选收窄、**已勾但上游没有的列单列出来并能一键清掉**（直接不画等于把惹祸的东西藏起来）、
  报错点名是哪个参数并列出上游现有哪些列。
  ⚠ 正在配的那个取数节点**自己不算**收窄来源：拿它自己的选择去收窄自己的候选，
  等于一取消勾选就再也勾不回来。
  ⚠ 这个标记同时挂在**单值**字段（切分的「目标列」是一个 `string`）与**数组**字段上，
  一律当多选渲染的话，单值字段会被存成数组——typecheck 与 lint 双双放行，要到后端才报
  「要填一段文字」。按 schema 的 `type` 分派：`string` → 下拉，`array` → 多选。
...
**落地形态是弹窗不是右抽屉**：`@dt/ui` 里没有 `DtDrawer`，为一个页面单造一个抽屉件
不划算，而这两处要的都是「盖上来看一眼再关掉」。按后端给的 **`kind` 显式派发**：
...
运行历史做成画布页的一个弹窗（不单开页面，理由同 §8.4）。点某次运行 → `?run_id=…`，
同一组件进入只读态：
...
**已落地**（151 条用例，建模侧行覆盖 93.8% / 支覆盖 87.2%）。四处与计划不同，理由
写在对应小节里：路由带资源段（§8.1）、结果与运行历史用弹窗而不是右抽屉（§8.4）、
组合式拆成十个而不是四个（§8.6）、评估视图只做「指标网格 + 真值/预测散点」，
残差直方图与明细表留给下一轮。

**三处只有真跑或真闸门才逮得到的**：

- `ref` 会把整张图深包成响应式 Proxy，而 `structuredClone` 对 Proxy 一律抛
  `DataCloneError`。图状态是整份替换的写法，**落第一个节点就炸**，浏览器里同样。
  改 `shallowRef`——整份替换本来也用不上深响应式。
- 删一份选中若分成「删节点」「删边」两次 commit，撤销键要按两次才退得回来，而第一次
  按下去画面纹丝不动，读起来就是撤销坏了。删除必须是**一次** commit，且空选中不入栈。
- 台账依赖的手写线形漏了第 3 期新加的 `model` 键。手写类型比真接口**宽松**时
  typecheck / lint / 单测全绿，只有 `dataset-shapes` 那条契约测试拦得住。
...
**我们自己踩的（二期上线后现场逮到的三条，处置写在 §8.2 / §8.3）**：

| # | 我们的做法 | 后果 | 现在怎么做 |
| --- | --- | --- | --- |
| A | 保存前整图校验**写了但全仓零调用**（与参考仓第 7 条同款） | 问题只有按下「运行」才由后端拦下，而 `describeError` 只取 `message`，逐条定位信息全丢——用户看到的是「配置错误」四个字 | 图一改就防抖校验、问题逐条列出并点得进卡片、运行前再校一次 |
| B | 下游列候选只按台账列，不按上游取数挑的列收窄 | 能勾到一列上游根本不产出的列，报错落在下游节点上；用户读成「取数必须把列选全」 | 候选按 `known_keys_by_node` 同一口径收窄；已勾但上游没有的列单列出来并能一键清掉 |
| C | 必填的 `table_code` 没有长度下限 | 空串是合法 `str`，一个还没选台账的取数节点整份图校验全绿，跑到那一步才报「台账不存在」，整次运行失败、下游全 skipped | `table_field` 带 `min_length=1`，并由算子目录契约测试守住 |
