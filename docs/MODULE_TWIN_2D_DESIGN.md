...
`chrome.ts`）**继续产参考尺度**，缩放只在 `presets/nodes.ts` 合成
`TWIN_2D_BUILTIN_NODE_STYLES` 时过一道 `twin2dScaleNodeStyle()`（`presets/scale.ts`）。
...
- **电路符号族（`circuit.ts` 那 8 枚）不缩。** 它们本就是符号尺度（最小的接线点 6×6，
  ...
  168:40，这一处比例变化是接受的。
- **传感器药丸（`sensors.ts`）不缩。** 它不是节点样式，是贴在节点外沿的一枚标注，
  观感基准是「读得清」而不是节点占地；而它的基准字号 16 与单位字号 12.48 缩完会双双撞
  上 12px 地板、两级并成一级，等于把药丸的排版层级抹平。
