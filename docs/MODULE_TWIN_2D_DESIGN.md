...
└── src/
    ...
    │   ├── scale.ts                  出厂尺度：**长度类**取值等比缩到 1080p 观感档；出厂面的带尺度工厂也在这儿（§7.13）
    │   ├── nodes.ts                  汇总面：五族接成一张有序清单 + 一张 id → 样式
    │   ├── circuit.ts                8 枚电路符号（GB/T 4728，§6.2）
    │   ├── edges.ts                  5 种预置连线样式
    │   └── sensors.ts                4 种预置传感器药丸（**参考尺度**，发货走 scale.ts 那支，§7.13）
    │
    └── render/
...
上面 100 行量的是**参考尺度**——逐值复刻参考项目。但参考项目那套取值是在它自己的画布
比例下定的：本模块的缺省画布是 1280×720，铺到一块 1080p 大屏上整体放大 1.5 倍，于是
一个 224×124 的设备卡在屏上占掉 336×186、18px 的显示名渲成 27 物理像素——用户在 1080p
上配组态时，第一反应就是「内置件比屏幕大了一号」。

**处置：内置预置出厂时整体缩一档，比例 `TWIN_2D_PRESET_SCALE = 0.75`。**
取 0.75 而不是 0.7：它是 4 的倒数的三倍，224 / 196 / 124 / 112 / 140 这批以 4 为公约数的
取值缩完还是整数或半像素，尺寸表读得出来也断言得了；而 0.7 会把几乎每一个数变成
一位小数的循环尾巴。再小一档（0.7）之后 18px 的显示名落到 12.6px，已经贴着地板，
留不出下一轮再调的余地。

#### 落点：一道变换，不是逐个改字面量

各族模块（`nodesSource.ts` / `nodesVessel.ts` / `nodesTerminal.ts` / `nodesMisc.ts` /
`chrome.ts` / `sensors.ts`）**继续产参考尺度**，缩放集中在两个落点，两处读的是同一个
`TWIN_2D_PRESET_SCALE`：

- **节点样式**在 `presets/nodes.ts` 合成 `TWIN_2D_BUILTIN_NODE_STYLES` 时过一道
  `twin2dScaleNodeStyle()`；
- **出厂面的传感器药丸**在 `presets/scale.ts` 的 `twin2dShippedSensorPill()` 里过一道
  （下面那条「两分」）。

> ⚠ 这条分工不是洁癖：`tests/twin2d-preset-fidelity.spec.ts` 那 2600 行锁的是**参考取值**，
> 把 0.75 揉进各族文件就等于把产品侧的一次调档写进「逐像素复刻」的那份谱里，
> 从此谱里的每一个数都既不是参考项目的、也不是发货的。两张表要分开量：参考尺度那份在
> fidelity 谱与各族用例里，出厂尺度那份在 `tests/presets/nodes.test.ts` 的
> 「出厂尺度」一组。只锁一头的话，改缩放比与改各族取值都会有一头一声不吭。

`TWIN_2D_PRESET_SCALE` 与 `twin2dScaleNodeStyle` 都从包的 `index.ts` 导出：存量数据的
迁移脚本读的必须是这**同一个数**，另写一份字面量就是第二份真源。
...
| 量 | 缩 | 为什么 |
|---|---|---|
| 节点 `size`、图元 `size` / `minWidth` / `maxWidth` | ✔ 缩（节点盒再取整） | 节点盒是整数设计像素，留小数会让「预置过一遍归一化恒等」不再成立，而拖进画布走的正是归一化那条路 |
| `at` 的 `fill.inset` / `abs` 四边 / `anchor.dx,dy` / `perim.gap,dx,dy` | ✔ 缩 | 都是设计像素 |
| `layout.gap` / `layout.pad` / `radius` / `backdropBlur` | ✔ 缩 | 同上 |
| 阴影的 `x,y,blur,spread`（图元与 `rootPatch` 两处） | ✔ 缩 | 同上 |
| `font.size`（带地板，见下）/ `font.letterSpacing` | ✔ 缩 | 同上 |
| `repeat` 填充的 `width` / `gap` | ✔ 缩 | 它是一条纹样的几何，不缩会让缩小后的罐形底沿多出两个管接头 |
| `vec` 的 `px` 档几何、引脚符号的 `shape` 与 `length` | ✔ 缩 | 与样式 `size` 同一把尺子 |
| 外缘 `outline.r` | ✔ 缩 | 它是节点盒上的圆角半径，不缩线头会接在缩过的符号外面 |
| 虚线的 `dash` | ✔ 缩 | 它是路径上的长度，不是粗细 |
| **描边与边框的 `width`**、`txt.outline.width` | ✘ 不缩 | 0.75 档下 1px 的线掉到 0.75px，在非整数 DPR 上渲成一条发虚的灰线；而 `nonScaling` 的那几遍本就声明了「不随任何缩放走」，缩它自相矛盾 |
| 百分比 / `em` / `'auto'` 三种串形 | ✘ 不缩 | 本就是相对量——百分比归父盒、`em` 归字号，而两者都已经缩过一遍，再缩就是乘了两遍 |
| `abs` 的 `tx` / `ty` | ✘ 不缩 | 它们是相对**自身尺寸**的位移串，还夹着 `calc(-100% - 4px)` 这样的常量留白；缩它得在这里再写一个 CSS 解析器 |
| `perim.t`、端口 `at`、渐变坐标、`radial` 的 `cx/cy/r`、色标 `at` | ✘ 不缩 | 全是 0..1 归一值。缩了的表现是「底色变了」「引脚接在符号外面」，一处都不报错 |
| `vec` 的 `unit` 档几何 | ✘ 不缩 | 乘的就是已经缩过的实例盒 |
| `ico` 的 `draw` 一档（`viewBox` 与笔画） | ✘ 不缩 | 它自带 viewBox、是一套自洽坐标系，图标画多大由图元的 `size` 说了算 |
| `lineHeight`、`rootPatch.scale`、图元的 `scale`、`rotate` | ✘ 不缩 | 是倍数与角度，不是长度 |
| 动画 `durationMs`、`opacity`、颜色 | ✘ 不缩 | 与尺寸无关 |

**字号地板 `TWIN_2D_PRESET_MIN_FONT_SIZE = 12`：缩到 12px 就不再往下缩，本就小于 12px 的
原样留着**（地板只挡「越缩越读不清」这一个方向，不抬字号）。代价说清楚：参考尺度里
15px 与 12px 那两级（悬浮卡的读数与说明字、角标的 15px）缩完并成同一级 12px，悬浮卡里
读数与说明字从此靠数字字体、强调色与字晕分层，不再靠字号。这是有意的取舍——
12px 已经是大屏上的可读下限。

#### 两处豁免

- **电路符号族（`circuit.ts` 那 8 枚）不缩。** 它们本就是符号尺度（最小的接线点 6×6，
  缩完 4.5×4.5 连点都点不中），没有一个字要读；而线宽按上表不缩，只缩盒会让 1.4px 的
  墨线相对变粗，GB/T 4728 的符号糊成一团。代价是设备卡与符号的相对比例从 224:40 变成
  168:40，这一处比例变化是接受的。
- **连线预置（`edges.ts`）不缩。** 连线上带的长度只有线宽与虚线节拍，前者按上表不缩，
  后者跟着线宽才有意义。

#### 一处两分：传感器药丸

药丸不是节点样式，是贴在节点外沿的一枚标注，所以它**不跟着 `nodes.ts` 那一道走**；
但它会被物化进 `node.layers`，与节点并排站着，所以真正落进图里的那一份必须跟着缩。
口径按面分成两半：

- **参考面不缩。** `twin2dSensorPill()` 与 `TWIN_2D_SENSOR_PILLS` 产的仍是参考尺度
  （16 / 16 / 12.48、pad `[1.92,8,1.92,8]`、gap 4.48）——§7.8 那 4 行逐像素谱锁的就是它，
  一格都不许动。
- **出厂面与实例侧照缩。** 编辑器往节点上加药丸走 `twin2dShippedSensorPill()`
  （`presets/scale.ts`），= 参考那一枚过一道 `TWIN_2D_PRESET_SCALE`；存量图里已经物化进
  `node.layers` 的那些由迁移脚本按**同一档**缩。两边漂开的表现是「新加的药丸比旁边那些
  大一号」，而两边取值单看都对、一处都不报错。

> ⚠ **落点 `at` 不缩。** 它是调用方在**实例坐标**里选的位置，出厂缩放只管药丸自己那点
> 几何；跟着缩就是同一个比例乘了两遍——把药丸挪出去 84px 之后再新建一枚同落点的，
> 会差出 25%。（迁移脚本缩 `at` 反而是对的：那是用户当年在**旧**坐标系里选的落点。）
>
> ⚠ **描边宽照旧不缩**（`PILL_BORDER_WIDTH` 恒 1），与上表「描边与边框的 `width`」同源。
> 一边缩一边不缩的表现是「旧药丸的边比新的细」，而两处取值单看都对。
>
> ⚠ **`TWIN_2D_SENSOR_PILLS` 是参考尺度**：往图里塞药丸一律走出厂那支。这张预置表在包外
> 眼下零消费者，所以不另配出厂版；哪天有人拿它直接落进 `layers`，图上就会出现大小两套
> 药丸——同样零报错。

**照缩要付的代价**：基准字号 16 与单位字号 12.48 缩完双双撞上 12px 地板，三片字号
`16 / 16 / 12.48` 并成 `12 / 12 / 12`，药丸自己的排版层级被抹平，读数与单位从此靠颜色
与字重分层、不再靠字号。**这个代价值得付**——不缩的话药丸的字会反超节点名：1080p 上
药丸 11.8 屏幕像素对节点名 10.0，而今天是 11.8 对 13.3，主次整个翻了个个儿。

#### 出厂尺寸对照表

| 样式 | 参考尺度 | 出厂尺度 | 显示名 | 主读数 |
|---|---|---|---|---|
| 四个源类（余热 / 蒸汽 / 空气能 / 太阳能） | 224×124 | **168×93** | 18 → 13.5 | 28 → 21（能效药丸 20 → 15） |
| 水箱 `water-tank` | 196×140 | **147×105** | 18 → 13.5 | 30 → 22.5 |
| 分集水器 `manifold` | 224×126 | **168×95** | 18 → 13.5 | 30 → 22.5 |
| 三个末端（洗浴 / 采暖 / 空调） | 196×112 | **147×84** | 18 → 13.5 | 32 → 24 |
| 板式换热器 `heat-exchanger` | 154×154 | **116×116** | 17 → 12.75 | — |
| 文字标注 `label` | 224×50 | **168×38** | 18 → 13.5 | — |
| 8 枚电路符号 | 40×20 等 | **不变** | — | — |
| 两枚外挂件（角标 / 外置显示名） | — | — | 角标 15 → **12（地板）**、外置名 18 → 13.5 | — |
| 传感器药丸（出厂那支） | — | — | 三片 16 / 16 / 12.48 → **12 / 12 / 12** | gap 4.48 → 3.36、pad 8 → 6、边框 1 **不缩** |

（悬浮卡里 12px 那几段说明字、`caption` 的「输出 / kWh」原样 12px——它们本就在地板上。）

#### 对存量图的影响

节点实例的 `w/h` 落在**文档**里，`centerBoxOf` 只在它是 0 时才回落样式的 `size`。所以：
**用户手动改过尺寸的节点一格不动，从没改过的（落库为 0）跟着变小**——后者正是这次
调档要的效果。要连改过尺寸的那批一起缩，才需要一支迁移脚本，而它读的必须是
`TWIN_2D_PRESET_SCALE` 这同一个数。

⚠ 已经物化进 `node.layers` 的传感器药丸**没有这条回落**：它的字号、内边距与落点都是
文档里的字面量，预置换档一格都吹不到它。所以带药丸的存量图**必须**过一遍迁移，否则
旧药丸停在参考尺度、新加的是出厂尺度，两套并排站着。
