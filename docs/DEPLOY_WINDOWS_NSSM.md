...
把整套平台装在**一台 Windows 机器**上：九个部署单元（含边缘网关 nginx）各做成
...
外部依赖（不属于九个部署单元，但都装在同一台机器上）：

| 组件 | Windows 服务名（默认） | 监听 | 说明 |
|---|---|---|---|
| PostgreSQL + TimescaleDB | `postgresql-x64-17` | `127.0.0.1:5432` | 一库多 schema，六份迁移各管各的 |
| Redis | `Memurai` 或自建 | `127.0.0.1:6379` | **必须 ≥ 6.2**，理由见 §2.4 |
| 对象存储（MinIO）| `dt-minio`（§4.4 也用 nssm 装）| `127.0.0.1:9000` | 素材字节面 |

⚠ **除边缘那个端口（本部署 82）与 4840-4859 外，一律绑 `127.0.0.1`。** 后端服务默认绑 `0.0.0.0`，
而 platform / opcua / realtime 完全依赖边缘注入的签名身份头来认人——把 8005
暴露在厂区网上，等于给了一条绕过闸 1 的路。绑定由每个服务的
...
| `auth-migrate` … `assistant-migrate`（6 个一次性作业）| §6 的六条命令，**人跑** |
| `minio-init` | §4.4 的建桶与匿名前缀，**人跑** |
| 其余 9 个服务 | 同名的 9 个 Windows 服务 |
...
```powershell
C:\DigitalTwin\app\server\.venv\Scripts\python.exe -c "import auth_server, platform_server, collector_server, opcua_server, realtime_hub, ai_assistant; print('ok')"
```
...
六个 schema（`auth` / `platform` / `collect` / `opcua` / `realtime` / `assistant`）
都在**同一个库**里，由各自的迁移链创建，见
[ADR-0003](adr/0003-一库多schema且写独占读放行.md)。本部署用的是库 `dt_db` +
`postgres` 账号（六份 `.env` 里已经填好，§5.6）：
...
改账号要同时改六份 `.env` 的 `*_POSTGRES_USER` / `_PASSWORD`——**改一份忘一份的
表现是那一个服务起不来，而别的五个都好**。

### 4.2 Redis

无口令实例请**只监听 127.0.0.1**；配了口令的把它填进每个服务的
`<前缀>_REDIS_PASSWORD`——本部署配了口令，六份 `.env` 里已经填好且取值一致
（§5.6）。⚠ 口令里带 `@` 之类的字符不用转义也不要加引号：连接串是代码拼的，
用户名与口令都过一遍百分号编码（`lib/config/base.py`）。自己在 `redis-cli`
或 `psql` 里手工验证时才需要按 shell 的规矩引起来。⚠ 九个服务共用同一个实例、同一个 `db`——快照、租约、
消费组、pub/sub 都在里面，不要为了「隔离」给某个服务换 `db`，那会让
publisher 读不到 collector 写的快照。
...
> 沿用仓库里生成好的六份 `.env`（§5.6）时跳过本节——那六份里的密钥已经生成过，
> 且六份取值一致。本节留给**轮换**，以及在 Windows 上从 `.env.example` 从头配的场合。
...
| `AUTH_EDGE_SIGNING_SECRET` | auth / platform ×3 / opcua / realtime / assistant | 登录成功，但业务面一律 401 |
| `AUTH_EDGE_SERVICE_KEY` | auth / platform ×3 / opcua / realtime / assistant / **collector** / **nginx 的 `X-Service-Key`** | 全站 403（nginx 那份写错）；采集永久空转（collector 那份写错）|
| `COLLECT_CREDENTIAL_SECRET` | platform ×3（`PLATFORM_COLLECT_CREDENTIAL_SECRET`）| 换过之后旧密文解不开：计划按「未配置凭据」下发并响亮记日志，界面上重填口令即恢复 |
| `OSS_ACCESS_KEY` / `OSS_SECRET_KEY` | MinIO 自己的 root 凭据 / `mc` / platform ×3 | 直传凭证签得出来、浏览器一传就 403 |

⚠ **platform 的三个角色共用一份 Settings，四项对象存储配置每个角色都要给全**，
哪怕 worker 与 publisher 一个字节都不读。只给 api 的表现是 **api 健康、worker 与
publisher 无限重启**，日志里只有一行「配置错误：PLATFORM_OBJECTSTORE_* Field required」。

### 5.3 跨服务地址：把服务名换成 127.0.0.1

代码里的默认值是**容器里的服务名**，在 Windows 上一个都解析不了。这七项必须逐个写：
...
| `ASSISTANT_MODEL_BASE_URL` | 供应商端点 | 按供应商填，走外网 |
...
| 变量 | 值 | 为什么 |
|---|---|---|
| `<前缀>_APP_HTTP_HOST` | `127.0.0.1` | 默认 `0.0.0.0`；后端不能暴露在厂区网上（§1.1）。前缀 = `AUTH_` / `PLATFORM_` / `OPCUA_` / `REALTIME_` / `COLLECT_` / `ASSISTANT_` |
...
仓库里已经生成好六份**完整**的 `.env`，取值口径就是 `docker/compose.yml`，
Windows 该改的地方（回环地址、Windows 路径、角色）都已经改过：

| 文件 | 变量数 | 还要你填 |
|---|---|---|
| `services\auth-server\.env` | 44 | `AUTH_SEED_ADMIN_PASSWORD`（首次建管理员）|
| `services\platform-server\.env` | 108 | 现场 EMS 四项（`PLATFORM_SQLSERVER_*`）|
| `services\collector-server\.env` | 40 | —— |
| `services\opcua-server\.env` | 35 | —— |
| `services\realtime-hub\.env` | 39 | —— |
| `services\ai-assistant\.env` | 61 | ——（接模型另见 §5.7）|

**只剩 5 项待填，逐个搜 `<` 就能找全**——没填的值都写成 `<说明>` 的样子，
留着不改的话服务会在第一秒响亮失败，而不是带着一个错值跑起来。

⚠ **`.env` 不在版本库里**（`.gitignore` 挡着，这是对的）。所以 Windows 机器上
`git clone` 出来的检出**不会有这六份文件**：要么从生成它们的机器上拷过去，
...
| 项 | 取值 | 依据 |
|---|---|---|
| 六处 `*_POSTGRES_HOST/PORT/USER/DB` | `127.0.0.1:5432`、`postgres`、`dt_db` | §4.1 |
| 六处 `*_POSTGRES_PASSWORD`、`*_REDIS_PASSWORD` | 现场口令，**六份一致** | §4.1 / §4.2 |
| 六处 `*_REDIS_HOST/PORT` | `127.0.0.1:6379`（同实例同 `db`）| §4.2 |
| 六处 `*_APP_HTTP_HOST` | `127.0.0.1` | §1.1 |
| 六处跨服务地址（`*_BASE_URL`）| `http://127.0.0.1:<端口>` | §5.3 |
...
⚠ **库与 Redis 那几项六份必须一模一样**（同一个库、同一个账号、同一个 Redis
实例与 `db`，六个 schema）。改一份忘一份的表现是那一个服务起不来，而别的五个都好；
Redis 换了 `db` 更隐蔽——服务全都健康，只是 publisher 读不到 collector 写的快照。

⚠ **platform 的三个角色共用这一份 `.env`**，里面写的是 `PLATFORM_APP_ROLE=api`；
worker 与 publisher 由 nssm 的环境变量覆盖（环境变量压过 `.env`，§7.2）。
...
容器里这是五个一次性作业，这里是**人跑的六条命令**。顺序无所谓，但**必须在起
...
# 期望列出 auth / platform / collect / opcua / realtime / assistant
```
...
所以要一次给全。角色只能从这里给——三个 platform 服务共用同一份 `.env`，
**环境变量压过 `.env`**，这正是我们要的——那份 `.env` 里写着 `PLATFORM_APP_ROLE=api`，
worker 与 publisher 靠这里的环境变量把它顶掉。
...
Get-ChildItem $svc\*\.env | Select-Object FullName  # ③ 六份 .env 在不在
...
九条，逐条跑完再交付。每条都给了期望值，**对不上就停下来查**，不要凑合往下走。
...
```powershell
foreach ($u in 'auth','platform','opcua','realtime','assistant') {
  "{0,-10} {1}" -f $u, (curl.exe -s -o NUL -w "%{http_code}" "$edge/api/v1/$u/health")
}
# auth / platform / opcua / realtime 必须 200；assistant 没部署时 502 是正常的
```
...
| 来源 | 位置 |
|---|---|
| 九个服务的 stdout/stderr | `C:\DigitalTwin\logs\dt-*.log` / `dt-*.err.log`（nssm 按 20MB 轮转）|
...
# 3) 迁移（六条，见 §6）——服务还在跑着，扩展步与旧代码兼容

# 4) 只要加过端点/权限码，就重跑 auth 种子（见 §6 的告诫）

# 5) 重启。worker/publisher/collector 先，api 与 edge 后
Restart-Service dt-platform-worker, dt-platform-publisher, dt-collector
Restart-Service dt-auth, dt-platform, dt-realtime, dt-opcua, dt-assistant
...
| 九份 `.env` + `nginx\conf` | 加密存放（含明文密钥）| 重建时要重新生成密钥 = 全体登出 + 数据源口令要重填 |
...
2. **升级是全量的。** 九个服务共用一个 `.venv`，没有容器那种「只换一个服务」。
...
4. **`platform-publisher` 全局单活。** 别为了「双保险」多装一个，两个进程只会抢
   同一把租约。
5. **没有 TLS。** 本部署的边缘是 `listen 82`。要 HTTPS 就在 `server` 块里加
   `listen 443 ssl;` 与证书路径，其余不变（Windows 版 nginx 带 `http_ssl_module`）。
6. **手工同步的两处**：`nginx.conf` 与 `.env`。仓库里的模板改了，这台机器不会
   自己跟着变——每次升级都要对一遍 §8.1 的差异表。
...
□ 有新迁移？ 六个服务目录逐个 alembic upgrade head
□ 加过端点或权限码？ auth-server 目录下 python -m scripts.seed   ← 漏了就是那一片全 403
□ 加过新配置项？ 对照 docker/compose.yml 补进对应服务的 .env     ← 漏了就是启动即失败
