...
|---|---|---|
| 函数 | **50 行** | 抽子函数 |
| 组合式函数 | 150 行 | 按关注点拆多个 `useXxx` |
| store 的 setup 函数 | 按**类**算，300 行 | 拆 store 或把某个关注点抽成组合式函数 |
| 单文件组件 | **300 行**（含模板与样式） | 拆子组件或把逻辑抽进组合式函数 |
| 模板嵌套 | **6 层** | 拆子组件 |
| `props` | 10 个 | 相关的聚成一个对象 prop |
| 圈复杂度 | 10 | 拆分支 |

⚠ 单文件组件超过 300 行时，几乎总是因为**逻辑写在了组件里**。逻辑抽进组合式函数不只是为了行数——组合式函数能被独立单元测试，而组件只能被挂载测试。
