...
`domain/` 装的是**多个服务共用、但确实带项目名词**的领域代码（台账公式引擎、点位历史编解码）。它进不了 `lib`（违反零项目名词铁律），也不该复制到每个服务里。入场券极窄：**必须已经有 ≥ 2 个服务真实消费它**，"将来可能共用"不构成理由。完整约束见 [ADR-0004](../adr/0004-server分三层且domain承载领域共享包.md)。
...
| 台账公式求值、时序编解码 | `domain/`（见 [ADR-0004](../adr/0004-server分三层且domain承载领域共享包.md)） | 带项目名词，进不了 `lib`；被多服务消费，不该复制 |
