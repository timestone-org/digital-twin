...
### 5.2.2 3D 手动操作模式分成编辑态与运行态两份

孪生编辑器顶栏提供「轨道操作 / 游戏操作」切换，这一档只属于当前编辑会话，
不写进孪生文档。游戏操作单击视口进入 Pointer Lock，鼠标控制朝向，WASD 按当前
水平视线前后左右平移，空格沿世界竖直轴上升、Shift 下降；转向与六向移动都按帧
做阻尼，松键与退出锁定后平滑停下。

`twin-view.config_json.navigationMode` 另行决定运行态预览与发布后大屏的操作方式，
是模块清单「运行态」组里的 3D 专属配置。缺失或未知值回落 `orbit`，保证存量大屏
继续沿用现有操作；它不跟随孪生编辑器顶栏的临时选择，避免作者为了摆放模型切换
操作方式时，顺手改变观看者的操作习惯。

### 5.2.3 读数字体是整屏缺省 + 节点覆盖

只有明确标记为实时指标读数的文字消费 `--font-digit`；日期、标识、说明文字与
坐标轴刻度不因为内容里含数字就自动改成装饰字体。平台缺省是 HarmonyOS Sans SC。

`chrome_json.card.digitFont` 给整屏定缺省，
`config_json.__cardStyle.digitFont` 给单个节点覆盖。两级沿用卡片外观的浅合并，
运行时把最终 id 解析成 `--card-digit-font`，再由模块宿主局部桥接到
`--font-digit`。因此同一张屏可以默认使用 DS-Digital，同时让某个节点显式改回
鸿蒙；管理页面不会继承到这项设置。

字体 id、面板标签与字体 token 映射同源于
`DASHBOARD_DIGIT_FONT_OPTIONS`，实际字体栈只定义在 `@dt/tokens`。新增字体只
扩目录与 token 并由宿主加载对应的已授权字体资源，不改模块。空值表示继承上级；
未知 id 不抛错、不残留旧字体，回退平台缺省。DS-Digital 以未修改原件随包提供，
缺字逐字回退默认字体；它是 shareware，商业交付前必须完成对应授权。
