...
| **P7** ✅ | docx 里的插图也收进来 | P3 | docx 也看得见图 |

⚠ P6 只做了「说实话」那一半：能力面现在分得开「重排没接」与「重排接了但
一直排不成」。父块回填与查询侧改写仍旧**等复量**——理由见 §P6。

⚠ P7 是做完 P3 之后补的：P3 落地时以为「docx 里的图与正文的对应关系
python-docx 给不出来」，实测不成立（`a:blip/@r:embed` 就挂在 `w:p` 底下）。
不补的话，现场最常见的 docx 报告一张图都进不了对话。
