...
台账公式求值器保持纯同步。只要本批至少有一个模型声明 `should_batch`，
`record_compute` 就先空跑一遍目标行，把全部 `PREDICT` 调用按
`(公式标识, 实参元组, 当前行时刻)` 收集；随后按公式整批调用 `predict_many`，
把结果写入本批次的 `ModelMemo`，再执行正常求值。

备忘键包含一次调用的全部输入，因此命中与行序无关。收集阶段可能因公式依赖尚未
算出而漏掉调用；正常阶段遇到未命中时必须逐行现算，不能返回空值。这样批量相位只
改变性能，不改变结果，也不需要给公式列分深度或按调用序号回放。

收集阶段不写任何记录，避免占位值被落库。只有纯 JSON 模型时不启动这趟额外扫描；
与二进制模型混用时，具备 `predict_many` 的纯 JSON 调用也会顺带进入同一备忘。
推理链上的每一步都必须保持行数与顺序，否则整批结果会静默错位。
实现见 `apps/dataset/services/record_compute.py` 与
`apps/dataset/formula/analysis.py`；历史 ADR-0046 已由本节接管。
...
每期可独立交付、独立验收。PR 范围、服务边界与锁文件纪律以
[`engineering-workflow.md`](./agents/engineering-workflow.md) 为准。
迁移、鉴权、并发、对外契约逐行评审。
