...
**本期只有表达式档。** 参考实现还有一档 `analysis`（把实参交给一个 AI provider 求值），
本仓**整档不做**：没有 provider，落一个只有一种取值的 `kind` 列等于把一个死开关钉进表结构。
将来要加是**扩展步**：新增一个可空的 `kind` 列 + 一个可空的 `provider` 列，旧代码读到
NULL 照旧当表达式档。
