...
| 词 | 指什么 |
|---|---|
| **目标句柄**（handle） | 「跳到哪张屏」的那个字符串。**登录态是大屏 id**，将来公开态是目标屏的公开令牌 |
...
| 公开态 `PublicDashboard` | 一期**不接**（它也没有联动引擎，见 §6） | 不跳 |
...
公开页现在本来就不 provide 联动引擎，一条联动规则都不跑（这条既有事实值得单独记一笔：
公开页目前连显隐与弹窗联动都没有，不只是跳转）。

将来要让公开页也能跨屏切换时，走的是「**把句柄改写成目标屏的公开令牌**」这条路：
未发布的目标改写成空串，前端据此当作不可跳。那一步会让公开面第一次带上
「同一个键在不同宿主里装不同东西」的语义，是一次值得单独拍板的取舍，不塞进这一期。
...
1. **公开态跨屏切换**（§6 那条改写路线）。要不要做取决于公开链接是不是真在用。
...
| 公开面不含大屏 id | 后端契约测试：配了跳转规则的屏发布后，公开载荷里搜不到 `interactions`，也搜不到任何 uuid |
| 换屏必须换订阅 | §7.1 的必红用例 |
